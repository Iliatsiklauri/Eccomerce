...
export default function Card({ card }: { card: Product }) {
  return (
    <div className=" w-[240px] group gap-2 flex flex-col justify-between h-[280px] text-black p-1.5 rounded-lg transition-all duration-200 ease-in-out flex-shrink-0">
      <div className="relative w-full h-44 overflow-hidden flex-shrink-0 rounded-md">
        <div className="bg-black h-full w-full absolute z-10 opacity-0 group-hover:opacity-15 transition-all duration-200 ease-in-out cursor-pointer"></div>
        <CartButton />
        <Image
          src={`${card.image}`}
